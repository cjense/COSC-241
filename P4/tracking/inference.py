    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             Radar has fuzzy reading - you can also get the signal "I just ate a ghost"
             So then you know with 100 percent certainty the ghost is in the prison cell
        ...
        # Go through each legal position, calculate the probability that the ghost is there given noisyDistance
        if noisyDistance is None:  # Special case
        ...
        for position in self.legalPositions:
            if noisyDistance is None:  # Special case
                ...
                # Weight each position's probability based on the likeliness it gave me the evidence I have
                self.beliefs[position] = self.beliefs[position] * emissionModel[distance]
        self.beliefs.normalize()
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            for newPos in self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)):
                newBeliefs[newPos] += self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))[newPos] * \
                                      self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for particlePos in self.particles:
            distance = util.manhattanDistance(particlePos, pacmanPosition)
        ...
        if distribution.totalCount() is 0:  # Special case 2
            self.initializeUniformly(gameState)
            return
        self.particles = []

        for i in range(self.numParticles):
            self.particles.append(util.sample(distribution))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # Create permutations of ghost arrangements, shuffle, sample self.numParticles from them
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistance is not None:
                    ...
                    probability *= emissionModel[distance]
        ...
        self.particles = []
        for i in range(self.numParticles):
            if distribution.totalCount() != 0.0:
                self.particles.append(util.sample(distribution))
        distribution.normalize()
